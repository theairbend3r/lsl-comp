...
def main(
    ...
    ] = "192.168.137.128",  # 192.168.137.128 local bound address as configured in most NSP settings
    inst_port: Annotated[
        int,
        typer.Option(
    ...
    ] = "",  # value -> "" for nsp, value ->"127.0.0.1" for Nplay
    ...
    * 1024,
    ...
    ] = False,  # Use local time.time if you stay on one machine, else it will introduce time mismatch in LSL
):
    ...
    comps = {
        ...
        "LSL_OUTLET": LSLOutletUnit(stream_name="lsl_nsp", stream_type="EMG"),
        "NS2": FilterOnKey(key="ns2"),
        "test": DebugLog(),
    }

    conns = ((comps["SRC"].OUTPUT_SIGNAL, comps["LSL_OUTLET"].INPUT_SIGNAL),)
